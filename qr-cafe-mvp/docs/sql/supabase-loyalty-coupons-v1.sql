...
begin
  ...
  v_paid_amount := greatest(
    0,
    coalesce(v_order.total_price, 0) - coalesce(v_order.used_points, 0) - v_coupon_discount
  );
  ...
  -- revoke auto-issued coupons generated by this order finalize
  update public.customer_coupons c
  set status = 'cancelled',
      updated_at = v_now
  where c.id in (
...
-- ---------------------------------------------------------
-- 13) Admin selected-customer coupon issue RPCs
-- ---------------------------------------------------------
