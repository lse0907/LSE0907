...
- 베타 테스터: OPS가 승인한 첫 혜택 매장의 기본 구독 40% 지속 할인.
- 온라인 선결제 베타 참여: OPS가 별도 승인한 경우 같은 혜택 매장의 옵션도 40% 지속 할인.
- 베타 할인은 결제 시점 정상가격에 적용하며 장기·다매장 가격할인과 중복하지 않음.
- 구독 중단·재구독·결제 실패만으로 베타 자격을 제거하지 않으며, 추가 매장과 양수인에게 자동 승계하지 않음.
...
- 결제 금액 계산 규칙(서버 강제):
  - `/api/billing/quote`가 가격 정책, 베타 테스터, 추가 매장, 기간 할인을 계산하고 서버 견적을 저장.
