...
export function CustomerOrderNotificationCard({ storeId, orderId, accessToken, preview = false }: Props) {
  ...
  const save = async (action: "subscribe" | "status" | "disable_device", subscription: SavedSubscription) => {
    ...
    return body as { ok: true; subscribed?: boolean };
  };

  useEffect(() => {
    if (preview) {
      setState("idle");
      setMessage("정식 연결 전 미리보기입니다. 실제 권한 요청이나 알림 발송은 하지 않습니다.");
      return;
    }
    ...
    let disposed = false;
    const restoreExistingDeviceNotification = async () => {
      // Permission and a PushSubscription belong to this browser device, while
      // the order link is intentionally short-lived. Attach an existing device
      // subscription to each new order without asking the customer again.
      ...
      try {
        if (!disposed) {
          setState("working");
          setMessage("이 기기의 알림 설정을 주문에 연결하고 있습니다.");
        }
        ...
        const status = await save("status", saved);
        ...
        if (!disposed) {
          setState("error");
          setMessage(error instanceof Error ? error.message : "알림 설정을 확인하지 못했습니다.");
        }
      }
    };
    void restoreExistingDeviceNotification();
    return () => {
      disposed = true;
    };
  // The connection must be rechecked whenever a new order is created.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [accessToken, configured, orderId, preview, storeId, supported]);
