  ...
  return (
    ...
    <main className="privacyPage">
      <style jsx>{`
       ...
       `}</style>
       <style jsx>{`
         :global(.topBack){display:inline-flex;align-items:center;gap:7px;min-height:40px;padding:0 12px;border:1px solid #d8e1ec;border-radius:11px;background:#fff;color:#334b6a;font-size:11px;font-weight:850;text-decoration:none}:global(.topBack) span{font-size:14px}
         ...
         .serviceHead{display:flex;justify-content:space-between;gap:16px;align-items:center}.serviceHead h2{margin:3px 0 0;font-size:17px}.serviceHead small{color:#bcd1ed}
       ...
       `}</style>
       <div className="brandBar"><RionBrand product auth /><Link className="topBack" href={backHref}><span aria-hidden="true">←</span>{backLabel}</Link></div>
      <section className="hero">
        <header className="top">
           <div><span className="eyebrow">ACCOUNT &amp; PRIVACY</span><h1>계정·개인정보 관리</h1><p className="sub">내 정보와 동의 상태를 확인하고 필요한 요청을 관리합니다.</p></div>
        </header>
      </section>
      ...
      {!loading && data ? <>
        <section className="serviceCard">
          <div className="serviceHead"><div><small>현재 계정의 서비스</small><h2>관리할 서비스를 선택하세요</h2></div><small>하나의 이메일 계정</small></div>
