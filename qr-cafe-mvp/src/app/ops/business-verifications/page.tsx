...
export default function OpsBusinessVerificationsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <main className={`opsReview ${embedded ? "embedded" : ""}`}>
      {!embedded ? <header><div><RionBrand product inverse /><span>BUSINESS VERIFICATION</span></div><nav className="opsHeaderActions"><Link href="/ops"><OpsIcon name="home" />OPS 홈</Link><button onClick={() => void load()}><OpsIcon name="refresh" />새로고침</button></nav></header> : null}
