...
export default async function PrivacyPage({ searchParams }: { searchParams: Promise<{ audience?: string }> }) {
  ...
  return (
    <LegalPolicyPage title={isSignupNotice ? `${isOwner ? "사업자 회원" : "고객"} 가입 개인정보 처리 안내` : "개인정보 처리방침 검토본"} description="가입·서비스 이용 과정에서 처리하는 정보와 이용자의 선택권을 설명합니다.">
      <section><h2>1. 가입 시 처리하는 정보</h2>{isSignupNotice ? (isOwner ? <ul><li>이메일과 인증정보</li><li>가입자 이름과 업무용 전화번호</li><li>만 19세 및 사업자 대표자·위임 담당자 확인</li><li>문서 종류·버전과 확인·동의 이력</li><li>추천코드는 선택사항이며 입력한 경우 연결 이력을 처리합니다.</li><li>개인 거주지 주소와 개인 상세주소는 수집하지 않습니다.</li></ul> : <ul><li>이메일과 인증정보</li><li>이름 또는 닉네임</li><li>만 14세 이상 확인</li><li>문서 종류·버전과 확인·동의 이력</li><li>전화번호는 선택사항이며 미입력으로 가입할 수 있습니다.</li></ul>) : <ul><li>고객: 계정정보, 선택 전화번호, 주문·혜택 이용기록</li><li>사업자 회원: 계정정보, 사업자·매장정보, 구독·결제·운영기록</li><li>비회원: 주문 처리에 필요한 주문정보와 필요한 경우 주문별 연락처</li><li>직원·매니저: 업무용 식별정보와 권한·감사기록</li></ul>}</section>
