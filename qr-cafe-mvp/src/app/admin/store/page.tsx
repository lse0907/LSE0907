...
function AdminstorePageInner() {
  ...
  return (
    <main className={`wrap ${isDirty ? "wrapHasSaveBar" : ""}`.trim()}>
      <style jsx global>{`
        :root {
          --bg: #f6f7f9;
          --card: #ffffff;
          ...
          <section className="card">
            ...
            <div className="inlineGrid">
              <div className="field">
                <div className="label">
                  대표 이미지 업로드
                  <span className={`uploadStatusBadge ${uploadingMain ? "uploadStatusBadgeBusy" : (draft as any).mainImage ? "uploadStatusBadgeOn" : ""}`.trim()}>
                    {uploadingMain ? "업로드 중" : (draft as any).mainImage ? "등록" : "미등록"}
                  </span>
                </div>
                <input
                  className="input"
              ...
              <div className="field">
                <div className="label">
                  로고 이미지 업로드 (선택)
                  <span className={`uploadStatusBadge ${uploadingLogo ? "uploadStatusBadgeBusy" : (draft as any).logoImage ? "uploadStatusBadgeOn" : ""}`.trim()}>
                    {uploadingLogo ? "업로드 중" : (draft as any).logoImage ? "등록" : "미등록"}
                  </span>
                </div>
                <input
                  className="input"
