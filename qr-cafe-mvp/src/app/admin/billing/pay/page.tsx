...
type Runtime = { baseStatus: string; addonStatus: string; basePaidUntil: string | null; addonPaidUntil: string | null; lastPaidAt: string | null };
...
function BillingPayContent() {
  ...
  const [runtime, setRuntime] = useState<Runtime>({ baseStatus: "inactive", addonStatus: "inactive", basePaidUntil: null, addonPaidUntil: null, lastPaidAt: null });
  ...
  const refreshRuntime = useCallback(async () => {
    ...
    const [storeRes, baseRes, addonRes, paymentRes, keyRes] = await Promise.all([
      ...
      supabase.from("store_addons").select("prepay_addon_status,addon_paid_until").eq("store_id", storeId).maybeSingle(),
  ...
  return (
    <main className="billingWrap">
      ...
      <section className="statusPanel">
        ...
        <div><span>선결제 옵션</span><strong>{statusLabel(runtime.addonStatus)}</strong><small>만료 {dateText(runtime.addonPaidUntil)}</small></div>
