...
function statusClass(status: string) {
  return status === "active" ? "badge badgeOk" : "badge badgeWarn";
}
...
function BillingPayForm({ storeId, storeName }: { storeId: string; storeName: string }) {
  ...
  const baseRemainDays = calcRemainDays(runtime.basePaidUntil);
  const addonRemainDays = calcRemainDays(runtime.addonPaidUntil);

  const fmtRemain = (days: number | null) => {
    if (days == null) return "-";
    if (days === 0) return "만료";
    return `${days}일`;
  };
  ...
  return (
    <section className="card">
      ...
      <section className="summaryGrid" aria-label="구독 상태 요약">
        <article className="summaryCard">
          <div className="summaryHead">
            <h3>기본 구독</h3>
            <span className={statusClass(runtime.baseStatus)}>{statusLabel(runtime.baseStatus)}</span>
          </div>
          <p className="value">{fmt(runtime.basePaidUntil)}</p>
          <p className="muted">만료까지 {fmtRemain(baseRemainDays)} · 최근 {runtime.baseMonths ?? "-"}개월 플랜</p>
        </article>

        <article className="summaryCard">
          <div className="summaryHead">
            <h3>선결제 옵션</h3>
            <span className={statusClass(runtime.addonStatus)}>{statusLabel(runtime.addonStatus)}</span>
          </div>
          <p className="value">{fmt(runtime.addonPaidUntil)}</p>
          <p className="muted">만료까지 {fmtRemain(addonRemainDays)} · 최근 {runtime.addonMonths ?? "-"}개월 플랜</p>
        </article>
        ...
        <article className="summaryCard">
          ...
          <p className="value">{fmt(runtime.lastPaidAt)}</p>
          <p className="muted">사용 만료: {fmt(runtime.lastAfterPaidUntil)} · 잔여 {fmtRemain(remainDays)}</p>
        </article>
      </section>
      ...
      <div className="estimateCard">
        <div>
          ...
          <div className="estimateValue">{totalAmount.toLocaleString()}원</div>
        </div>
        <ul className="breakdown" aria-label="결제 금액 구성">
          <li>기본 구독: {payBase ? `${runtime.basePrice.toLocaleString()}원 × ${planMonths}개월` : "미포함"}</li>
          <li>선결제 옵션: {payAddon ? `${runtime.addonPrice.toLocaleString()}원 × ${planMonths}개월` : "미포함"}</li>
          <li>최근 결제개월: {runtime.lastPlanMonths ?? "-"}개월</li>
        </ul>
      </div>
...
const css = `
  ...
  * { box-sizing:border-box; }
  ...
  .summaryGrid { display:grid; gap:10px; grid-template-columns:repeat(3,minmax(0,1fr)); }
  .summaryCard { border:1px solid var(--line); border-radius:12px; padding:12px; display:grid; gap:6px; background:#fcfcff; }
  .summaryHead { display:flex; justify-content:space-between; align-items:center; gap:6px; }
  .summaryHead h3 { margin:0; font-size:14px; }
  .value { margin:0; font-size:14px; font-weight:800; }
  .badge { border:1px solid #cbd5e1; color:#334155; border-radius:999px; font-size:11px; font-weight:800; padding:4px 8px; background:#f8fafc; }
  .badgeOk { border-color:#a7f3d0; color:#065f46; background:#ecfdf3; }
  .badgeWarn { border-color:#fed7aa; color:#9a3412; background:#fff7ed; }
  .estimateCard { border:1px solid var(--line); border-radius:12px; padding:12px; display:grid; gap:10px; background:#f8fbff; }
  .estimateValue { font-weight:900; font-size:22px; }
  .breakdown { margin:0; padding-left:18px; color:#334155; display:grid; gap:4px; font-size:13px; }
