...
const targetOptions: Array<{ key: ImportTarget; title: string; summary: string; badge: string }> = [
  { key: "categories", title: "카테고리 등록", summary: "초기설정 순서에 맞게 categories.csv만 업로드합니다.", badge: "카테고리" },
  { key: "menus", title: "메뉴 등록", summary: "이미 등록된 카테고리에 menus.csv를 연결합니다.", badge: "메뉴" },
];
...
function AdminImportPageInner() {
  ...
  const validationSummary = needsCategoriesFile ? `카테고리 ${categoryRows.length}건` : `메뉴 ${menuRows.length}건`;
    ...
    for (let r = 1; r < menuRowsRaw.length; r += 1) {
      ...
      if (!availableCategoryNames.has(normalizeKey(categoryName))) {
        addError(nextErrors, {
          ...
          value: categoryName,
          solution: "현재 매장에 등록된 카테고리명과 동일하게 수정해 주세요.",
        });
        continue;
      }
      ...
      if (needsMenusFile) {
        ...
        if (availableCategoryNames.size === 0) {
          addError(nextErrors, {
            ...
            value: "",
            solution: "카테고리 관리에서 카테고리를 먼저 등록한 뒤 메뉴 파일을 다시 업로드해 주세요.",
          });
        }
  ...
  return (
    <main className="wrap">
      ...
      <style jsx>{`
        ...
        .headerActions { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; }
        ...
        .headerActions .btn { min-height: 36px; padding: 8px 9px; font-size: 12px; white-space: nowrap; }
        ...
        .guideUploadGrid { display: grid; grid-template-columns: 1fr; gap: 12px; }
        ...
        .templateAction { justify-self: start; min-height: 36px; padding: 8px 11px; }
        .uploadBox { border-top: 1px solid #e5e7eb; padding-top: 12px; display: grid; gap: 8px; }
        ...
        @media (max-width: 760px) {
          ...
          .templateGrid, .uploadGrid, .guideGrid, .resultGrid { grid-template-columns: 1fr; }
          .btn, .input { width: 100%; }
          .headerActions .btn { width: auto; min-height: 34px; padding: 7px 5px; font-size: 11px; }
      ...
      <section className="card">
        <div>
          ...
          <p className="muted" style={{ marginTop: 4 }}>초기설정 순서에 맞게 카테고리 또는 메뉴 중 하나만 선택해 업로드합니다.</p>
        </div>
        <div className="modeGrid">
          {targetOptions.map((option) => (
            ...
            >
              ...
              {target === option.key ? (
                <button
                  className="btn templateAction"
                  onClick={(event) => {
                    event.stopPropagation();
                    if (option.key === "categories") {
                      onDownloadCategoryTemplate();
                    } else {
                      onDownloadMenuTemplate();
                    }
                  }}
                  type="button"
                >
                  {option.key === "categories" ? "카테고리 템플릿 다운로드" : "메뉴 템플릿 다운로드"}
                </button>
              ) : null}
            </div>
          ))}
      ...
      <section className="card">
        ...
        {!storeId ? (
          <div className="statusCard statusError">관리자 홈에서 매장을 먼저 선택해 주세요.</div>
        ) : null}
        <div className="guideUploadGrid">
          {needsCategoriesFile ? (
          ...
          {needsMenusFile ? (
            <div className="miniCard">
              ...
              <div className="guideBox">category_name은 현재 등록된 카테고리명과 정확히 같아야 합니다.</div>
