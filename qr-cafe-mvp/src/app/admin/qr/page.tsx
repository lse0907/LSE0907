...
const TEMPLATE_OPTIONS: Array<{ key: TemplateKey; label: string; hint: string }> = [
  { key: "simple", label: "클린", hint: "기본형" },
  { key: "cafe_poster", label: "포토", hint: "이미지 강조" },
  { key: "premium_dark", label: "프리미엄", hint: "고급형" },
  { key: "soft_round", label: "라운드", hint: "카드형" },
];

const ACCENT_COLORS = ["#111827", "#7c3aed", "#b45309", "#047857", "#be123c"];

function templateLabel(key: TemplateKey) {
  return TEMPLATE_OPTIONS.find((option) => option.key === key)?.label || "클린";
}
...
function AdminQrPageInner() {
  ...
  function drawLogoMark(ctx: CanvasRenderingContext2D, img: HTMLImageElement | null, x: number, y: number, size: number, dark = false) {
    roundRect(ctx, x, y, size, size, Math.round(size * 0.22), dark ? "rgba(255,255,255,0.12)" : "#ffffff", dark ? "rgba(255,255,255,0.18)" : "#e5e7eb");
    if (img) {
      ctx.save();
      ctx.beginPath();
      roundedClipPath(ctx, x, y, size, size, Math.round(size * 0.22));
      ctx.clip();
      ctx.drawImage(img, x, y, size, size);
      ctx.restore();
    } else {
      ctx.fillStyle = dark ? "#ffffff" : "#111827";
      ctx.font = `900 ${Math.round(size * 0.34)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
      ctx.textAlign = "center";
      ctx.fillText("QR", x + size / 2, y + Math.round(size * 0.62));
      ctx.textAlign = "left";
    }
  }

  function drawQrBox(ctx: CanvasRenderingContext2D, img: HTMLImageElement, x: number, y: number, size: number, radius: number, stroke = "#e5e7eb") {
    const pad = Math.max(10, Math.round(size * 0.045));
    roundRect(ctx, x - pad, y - pad, size + pad * 2, size + pad * 2, radius, "#ffffff", stroke);
    ctx.drawImage(img, x, y, size, size);
  }

  function compactCounterDescription(ds: AdminQrDesignSettings, maxLines: number) {
    return (ds.counter_description || "")
      .split("\n")
      .map((x) => x.trim())
      .filter(Boolean)
      .slice(0, maxLines);
  ...
  async function createCounterPosterCanvas() {
    ...
    const logoBox = Math.round(64 * scale);
    const qrMax = counterPrintPreset === "a3_poster" ? 560 : counterPrintPreset === "a4_poster" ? 420 : 300;
    const qrSize = Math.min(Math.round(A4_W * 0.31), Math.round(posterH * 0.33), qrMax);
    ...
    for (let i = 0; i < copies; i++) {
      ...
      const title = ds.show_store_name ? storeName : ds.counter_title;
      const descLines = compactCounterDescription(ds, counterPrintPreset === "a3_poster" ? 3 : 2);

      if (ds.template_key === "simple") {
        ctx.fillStyle = "#ffffff";
        ctx.fillRect(0, topY, A4_W, posterH);
        drawLogoMark(ctx, logoImg, padding, topY + padding, logoBox, false);
        ctx.fillStyle = "#111827";
        ctx.font = `950 ${Math.round(46 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.textAlign = "center";
        ctx.fillText(ds.counter_title, A4_W / 2, topY + padding + Math.round(88 * scale));
        ctx.fillStyle = "#6b7280";
        ctx.font = `850 ${Math.round(20 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(title, A4_W / 2, topY + padding + Math.round(120 * scale));
        const qrX = Math.round((A4_W - qrSize) / 2);
        const qrY = topY + Math.round(posterH * 0.33);
        drawQrBox(ctx, qrImg, qrX, qrY, qrSize, Math.round(24 * scale));
        ctx.fillStyle = "#374151";
        ctx.font = `850 ${Math.round(20 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        descLines.forEach((line, idx) => ctx.fillText(line, A4_W / 2, qrY + qrSize + Math.round(72 * scale) + idx * Math.round(30 * scale)));
        ctx.textAlign = "left";
      } else if (ds.template_key === "cafe_poster") {
        const imgH = Math.floor(posterH * 0.62);
        const bottomH = posterH - imgH;
        if (heroImg) {
          const sw = heroImg.naturalWidth;
          const sh = heroImg.naturalHeight;
          const srcRatio = sw / sh;
          const dstRatio = A4_W / imgH;
          let cropW = sw;
          let cropH = sh;
          let cropX = 0;
          let cropY = 0;
          ...
          ctx.drawImage(heroImg, cropX, cropY, cropW, cropH, 0, topY, A4_W, imgH);
          const overlay = ctx.createLinearGradient(0, topY, 0, topY + imgH);
          ...
          g.addColorStop(0, ds.accent_color);
          g.addColorStop(1, "#111827");
          ctx.fillStyle = g;
          ctx.fillRect(0, topY, A4_W, imgH);
        }
        drawLogoMark(ctx, logoImg, padding, topY + padding, logoBox, true);
        ctx.fillStyle = "#ffffff";
        ctx.font = `950 ${Math.round(48 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(title, padding, topY + imgH - Math.round(82 * scale));
        ctx.font = `900 ${Math.round(28 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(ds.counter_title, padding, topY + imgH - Math.round(40 * scale));
        ctx.fillStyle = ds.accent_color || "#111827";
        ctx.fillRect(0, topY + imgH, A4_W, bottomH);
        const qrSizePhoto = Math.min(qrSize, Math.round(bottomH * 0.72));
        const qrX = A4_W - padding - qrSizePhoto;
        const qrY = topY + imgH + Math.floor((bottomH - qrSizePhoto) / 2);
        drawQrBox(ctx, qrImg, qrX, qrY, qrSizePhoto, Math.round(18 * scale), "rgba(255,255,255,0.18)");
        ctx.fillStyle = "#ffffff";
        ctx.font = `850 ${Math.round(19 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        descLines.forEach((line, idx) => ctx.fillText(line, padding, topY + imgH + padding + idx * Math.round(30 * scale)));
      } else if (ds.template_key === "premium_dark") {
        const g = ctx.createLinearGradient(0, topY, A4_W, topY + posterH);
        g.addColorStop(0, "#030712");
        g.addColorStop(1, "#1f2937");
        ctx.fillStyle = g;
        ctx.fillRect(0, topY, A4_W, posterH);
        ctx.strokeStyle = ds.accent_color;
        ctx.lineWidth = Math.max(3, Math.round(4 * scale));
        ctx.strokeRect(padding, topY + padding, A4_W - padding * 2, posterH - padding * 2);
        drawLogoMark(ctx, logoImg, Math.round((A4_W - logoBox) / 2), topY + padding + Math.round(22 * scale), logoBox, true);
        ctx.textAlign = "center";
        ctx.fillStyle = "#ffffff";
        ctx.font = `950 ${Math.round(42 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(title, A4_W / 2, topY + padding + Math.round(132 * scale));
        ctx.fillStyle = ds.accent_color;
        ctx.font = `900 ${Math.round(24 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(ds.counter_title, A4_W / 2, topY + padding + Math.round(172 * scale));
        const qrX = Math.round((A4_W - qrSize) / 2);
        const qrY = topY + Math.round(posterH * 0.38);
        drawQrBox(ctx, qrImg, qrX, qrY, qrSize, Math.round(20 * scale), "rgba(255,255,255,0.2)");
        ctx.fillStyle = "rgba(255,255,255,0.78)";
        ctx.font = `800 ${Math.round(19 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        descLines.forEach((line, idx) => ctx.fillText(line, A4_W / 2, qrY + qrSize + Math.round(70 * scale) + idx * Math.round(30 * scale)));
        ctx.textAlign = "left";
      } else {
        ...
        roundRect(ctx, cardX, cardY, cardW, cardH, Math.round(34 * scale), "#ffffff", "#fed7aa");
        drawLogoMark(ctx, logoImg, cardX + Math.round(28 * scale), cardY + Math.round(28 * scale), logoBox, false);
        ctx.fillStyle = "#111827";
        ctx.font = `950 ${Math.round(38 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(ds.counter_title, cardX + Math.round(28 * scale), cardY + Math.round(118 * scale));
        ctx.fillStyle = ds.accent_color;
        ctx.font = `900 ${Math.round(21 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(title, cardX + Math.round(28 * scale), cardY + Math.round(154 * scale));
        const qrX = Math.round(cardX + (cardW - qrSize) / 2);
        const qrY = cardY + Math.round(cardH * 0.36);
        drawQrBox(ctx, qrImg, qrX, qrY, qrSize, Math.round(28 * scale), "#fed7aa");
        ctx.fillStyle = "#6b7280";
        ctx.font = `850 ${Math.round(18 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.textAlign = "center";
        descLines.slice(0, 2).forEach((line, idx) => ctx.fillText(line, A4_W / 2, qrY + qrSize + Math.round(66 * scale) + idx * Math.round(28 * scale)));
        ctx.textAlign = "left";
      }

      if (ds.show_target_url) {
        ctx.fillStyle = ds.template_key === "premium_dark" ? "rgba(255,255,255,0.45)" : "#9ca3af";
        ctx.font = `800 ${Math.round(12 * scale)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif`;
        ctx.fillText(trimMiddle(counterUrl, 72), padding, topY + posterH - Math.round(18 * scale));
      }
  ...
  async function createTableSheetCanvases(nums: number[]) {
    ...
    for (const pageNums of pages) {
      ...
      for (let i = 0; i < loaded.length; i++) {
        ...
        const tableName = formatTableLabel(loaded[i].n);
        const brandText = ds.show_store_name ? storeName : ds.table_title;
        const logoSize = Math.min(38, Math.round(cardW * 0.14));

        if (ds.template_key === "premium_dark") {
          roundRect(ctx, x, y, cardW, cardH, 18, "#111827", ds.accent_color);
          if (ds.show_logo) drawLogoMark(ctx, logoImg, x + innerPad, y + innerPad, logoSize, true);
          ctx.fillStyle = "#ffffff";
          ctx.font = "950 18px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(brandText, x + innerPad + (ds.show_logo ? logoSize + 8 : 0), y + 30);
          ctx.fillStyle = ds.accent_color;
          ctx.font = "950 24px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(tableName, x + innerPad, y + 66);
          const darkQrSize = Math.min(qrSize, cardH - 118);
          const qrX = x + Math.floor((cardW - darkQrSize) / 2);
          const qrY = y + 88;
          drawQrBox(ctx, loaded[i].img, qrX, qrY, darkQrSize, 16, "rgba(255,255,255,0.18)");
          ctx.fillStyle = "rgba(255,255,255,0.68)";
        } else if (ds.template_key === "soft_round") {
          roundRect(ctx, x, y, cardW, cardH, 22, "#fff7ed", "#fed7aa");
          const innerX = x + 12;
          const innerY = y + 12;
          roundRect(ctx, innerX, innerY, cardW - 24, cardH - 24, 20, "#ffffff", "#fed7aa");
          if (ds.show_logo) drawLogoMark(ctx, logoImg, innerX + innerPad, innerY + innerPad, logoSize, false);
          ctx.fillStyle = "#111827";
          ctx.font = "950 23px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(tableName, innerX + innerPad, innerY + 48);
          ctx.fillStyle = "#6b7280";
          ctx.font = "850 14px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(brandText, innerX + innerPad + (ds.show_logo ? logoSize + 8 : 0), innerY + 76);
          const qrX = x + Math.floor((cardW - qrSize) / 2);
          const qrY = y + labelH + 12;
          drawQrBox(ctx, loaded[i].img, qrX, qrY, qrSize, 18, "#fed7aa");
          ctx.fillStyle = "#9ca3af";
        } else if (ds.template_key === "cafe_poster") {
          roundRect(ctx, x, y, cardW, cardH, 16, "#ffffff", "#e5e7eb");
          const heroH = Math.min(78, Math.round(cardH * 0.26));
          ctx.save();
          roundedClipPath(ctx, x, y, cardW, heroH, 16);
          ctx.clip();
          if (heroImg) {
            ctx.drawImage(heroImg, 0, 0, heroImg.naturalWidth, heroImg.naturalHeight, x, y, cardW, heroH);
          } else {
            const g = ctx.createLinearGradient(x, y, x + cardW, y + heroH);
            g.addColorStop(0, ds.accent_color);
            g.addColorStop(1, "#111827");
            ctx.fillStyle = g;
            ctx.fillRect(x, y, cardW, heroH);
          }
          ctx.fillStyle = "rgba(0,0,0,0.3)";
          ctx.fillRect(x, y, cardW, heroH);
          ctx.restore();
          if (ds.show_logo) drawLogoMark(ctx, logoImg, x + innerPad, y + 14, logoSize, true);
          ctx.fillStyle = "#ffffff";
          ctx.font = "950 17px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(brandText, x + innerPad + (ds.show_logo ? logoSize + 8 : 0), y + 39);
          ctx.fillStyle = "#111827";
          ctx.font = "950 22px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(tableName, x + innerPad, y + heroH + 32);
          const photoQrSize = Math.min(qrSize, cardH - heroH - 82);
          const qrX = x + Math.floor((cardW - photoQrSize) / 2);
          const qrY = y + heroH + 48;
          drawQrBox(ctx, loaded[i].img, qrX, qrY, photoQrSize, 14, "#e5e7eb");
          ctx.fillStyle = "#9ca3af";
        } else {
          roundRect(ctx, x, y, cardW, cardH, 16, "#ffffff", "#e5e7eb");
          if (ds.show_logo) drawLogoMark(ctx, logoImg, x + innerPad, y + 16, logoSize, false);
          ctx.fillStyle = "#111827";
          ctx.font = "950 18px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(brandText, x + innerPad + (ds.show_logo ? logoSize + 8 : 0), y + 28);
          ctx.fillStyle = "#111827";
          ctx.font = "950 24px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Noto Sans KR, sans-serif";
          ctx.fillText(tableName, x + innerPad, y + 62);
          const qrX = x + Math.floor((cardW - qrSize) / 2);
          const qrY = y + labelH;
          drawQrBox(ctx, loaded[i].img, qrX, qrY, qrSize, 14, "#e5e7eb");
          ctx.fillStyle = "#9ca3af";
        }
  ...
  async function refreshPrintPreview(canApply: () => boolean = () => true) {
    ...
    try {
      if (printTarget === "counter") {
        ...
        setPreviewNote(`${templateLabel(normalizeTemplateKey(effectiveDesign.template_key))} · ${COUNTER_PRINT_PRESETS[counterPrintPreset].label} · 실제 출력 기준`);
      } else {
        ...
        setPreviewNote(`${templateLabel(normalizeTemplateKey(effectiveDesign.template_key))} · ${TABLE_PRINT_PRESETS[tablePrintPreset].label} · ${Math.max(1, canvases.length)}장 중 1장`);
      }
    } catch (e: unknown) {
      if (!canApply()) return;
      setPreviewUrl("");
