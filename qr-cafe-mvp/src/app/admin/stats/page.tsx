...
function summarize(orders: OrderRecord[]): Summary { const out = { sales: 0, orders: 0, qty: 0, dineIn: 0, takeout: 0 }; for (const o of orders) { out.sales += Number(o.totalPrice || 0); out.orders += 1; out.qty += Number(o.totalCount || 0); if (o.mode === "dine-in") out.dineIn += 1; else out.takeout += 1; } return out; }
...
function AdminStatsPageInner() {
  ...
  const computedFetchRange = useMemo(() => {
    ...
    return { start: [safeStart, weekStart, monthStart].sort()[0], end: [safeEnd, weekEnd, todayKey].sort().slice(-1)[0] };
  ...
  const fetchFromDb = async () => {
    ...
    try {
      ...
      const { data: oData, error: oErr } = await supabase
        ...
        .eq("store_id", storeId).gte("order_date", start).lte("order_date", end).neq("status", "cancelled").order("created_at", { ascending: true });
      ...
      const { data: iData, error: iErr } = await supabase.from("order_items").select("id,order_id,menu_id,name,price,qty,store_id").eq("store_id", storeId).in("order_id", base.map((o) => o.id));
      ...
      for (const it of Array.isArray(iData) ? (iData as RawItemRow[]) : []) {
        ...
        arr.push({ id: getString(it, "menu_id") || getString(it, "id"), name: getString(it, "name"), price: Math.max(0, Math.round(getNumber(it, "price") || 0)), qty: Math.max(0, Math.round(getNumber(it, "qty") || 0)) });
        itemsByOrder.set(oid, arr);
      }
      setOrders(base.map((o) => { const its = itemsByOrder.get(o.id) || []; const computedCount = its.reduce((s, x) => s + (x.qty || 0), 0); const computedPrice = its.reduce((s, x) => s + (x.qty || 0) * (x.price || 0), 0); return { ...o, items: its, totalCount: o.totalCount || computedCount, totalPrice: o.totalPrice || computedPrice }; }));
      setDataMode("db");
    } catch (e: unknown) {
      console.error(e);
  ...
  const top5 = useMemo(() => {
    ...
    for (const o of monthlyOrders) for (const it of o.items || []) { const key = it.id || it.name; const prev = map.get(key); const lineSales = (it.price || 0) * (it.qty || 0); if (!prev) map.set(key, { name: it.name, qty: it.qty, sales: lineSales }); else { prev.qty += it.qty; prev.sales += lineSales; } }
    return [...map.entries()].map(([id, v]) => ({ id, ...v })).sort((a, b) => b.qty - a.qty || b.sales - a.sales).slice(0, 5);
  }, [monthlyOrders]);
  ...
  const rangeSummaryRows = useMemo(() => {
    ...
    for (const o of nonCanceled) { if (!inRange(o.orderDate, effectiveStart, effectiveEnd)) continue; const prev = bucket.get(o.orderDate) || { sales: 0, orders: 0, qty: 0, dineIn: 0, takeout: 0 }; prev.sales += o.totalPrice; prev.orders += 1; prev.qty += o.totalCount; if (o.mode === "dine-in") prev.dineIn += 1; else prev.takeout += 1; bucket.set(o.orderDate, prev); }
    const rows: Array<{ date: string } & Summary> = [];
    let cur = new Date(start);
    while (cur <= end) { const key = ymd(cur); rows.push({ date: key, ...(bucket.get(key) || { sales: 0, orders: 0, qty: 0, dineIn: 0, takeout: 0 }) }); cur = addDays(cur, 1); }
    return rows;
  ...
  const setPreset = (preset: "today" | "7days" | "month" | "lastMonth") => { const t = new Date(); if (preset === "today") { setRangeStart(ymd(t)); setRangeEnd(ymd(t)); } else if (preset === "7days") { setRangeStart(ymd(addDays(t, -6))); setRangeEnd(ymd(t)); } else if (preset === "month") { setRangeStart(ymd(startOfMonth(t))); setRangeEnd(ymd(t)); } else { const last = new Date(t.getFullYear(), t.getMonth() - 1, 1); setRangeStart(ymd(startOfMonth(last))); setRangeEnd(ymd(endOfMonth(last))); } setRangeExpanded(false); };
  ...
  const downloadRangeCsv = () => {
    ...
    if (!effectiveStart || !effectiveEnd) { alert("기간을 선택해 주세요."); return; }
    if (effectiveStart > effectiveEnd) { alert("시작일이 종료일보다 늦습니다."); return; }
    ...
    for (const o of list) { const orderItems = o.items?.length ? o.items : [{ id: "", name: "", price: 0, qty: 0 }]; for (const it of orderItems) rows.push([o.orderDate, formatTime(o.createdAt), o.displayNo, o.mode === "dine-in" ? "매장" : "포장", o.table ?? "", o.status, o.buzzerNo ?? "", o.requestNote ?? "", it.name, it.qty ? String(it.qty) : "", it.price ? String(it.price) : "", it.price && it.qty ? String(it.price * it.qty) : "", String(o.totalPrice), o.id, String(o.storeId || o.store_id || storeId)].map((x) => csvEscape(String(x))).join(",")); }
  ...
  const visibleRangeRows = useMemo(() => (isPaidSubscriber && rangeExpanded ? rangeSummaryRows : rangeSummaryRows.slice(Math.max(0, rangeSummaryRows.length - previewCount))), [rangeSummaryRows, rangeExpanded, isPaidSubscriber]);
  ...
  const renderMetricCard = (title: string, period: string, summary: Summary, locked = false) => (
    <div className={`card metricCard ${locked ? "locked" : ""}`}>
      <div className="cardHead"><div><h2 className="cardTitle">{title}</h2><div className="cardPeriod">{period}</div></div>{locked ? <span className="lockBadge">유료 전용</span> : <span className="miniBadge">포장 {takeoutRate(summary)}%</span>}</div>
      {locked ? <div className="lockBox"><strong>구독 후 확인</strong><button className="linkBtn" type="button" onClick={goBilling}>구독 관리</button></div> : <><div className="salesLine">{formatWonCompact(summary.sales)}</div><div className="metricLine">주문 {summary.orders}건 · 수량 {summary.qty}개</div><div className="metricSub">객단가 {formatWon(avgOrder(summary))} · 매장 {summary.dineIn} / 포장 {summary.takeout}</div></>}
    </div>
  );
  ...
  return (
    <main className="wrap">
      ...
      <style jsx>{`
        .wrap{max-width:1100px;margin:0 auto;padding:14px;display:grid;gap:10px}.topbar{display:grid;gap:8px}.titleRow{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap}.h1{margin:0;font-size:28px;font-weight:950;letter-spacing:-.02em}.metaRow,.btnRow,.quickRow{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.pill,.miniBadge,.lockBadge{border:1px solid var(--line);background:#fff;border-radius:999px;padding:6px 9px;color:var(--muted);font-size:12px;font-weight:900;white-space:nowrap}.lockBadge{background:#fff7ed;border-color:#fed7aa;color:#9a3412}.btn,.quickBtn,.linkBtn{border:1px solid var(--line);background:#fff;border-radius:12px;cursor:pointer;font-weight:900}.btn{padding:10px 13px}.quickBtn{padding:8px 10px;font-size:13px}.btnPrimary{background:var(--brand);border-color:var(--brand);color:#fff}.linkBtn{padding:7px 9px;font-size:12px}.card{background:var(--card);border:1px solid var(--line);border-radius:var(--radius);padding:12px;box-shadow:0 1px 0 rgba(0,0,0,.03)}.notice{display:flex;justify-content:space-between;align-items:center;gap:10px;background:#fffbeb;border-color:#fde68a}.noticeText{margin:0;color:#92400e;font-size:13px;font-weight:850;line-height:1.35}.summaryGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.cardHead{display:flex;justify-content:space-between;align-items:flex-start;gap:10px}.cardTitle{margin:0;font-size:16px;font-weight:950;line-height:1.15}.cardPeriod{margin-top:5px;color:var(--muted);font-size:12px;font-weight:850;line-height:1.15}.salesLine{margin-top:9px;font-size:27px;font-weight:950;letter-spacing:-.02em;line-height:1.05;white-space:nowrap}.metricLine{margin-top:8px;font-size:13px;font-weight:950}.metricSub{margin-top:4px;color:var(--muted);font-size:12px;font-weight:850;line-height:1.25}.locked{background:linear-gradient(180deg,#fff,#fafafa)}.lockBox{margin-top:14px;display:flex;justify-content:space-between;align-items:center;gap:10px;color:var(--muted)}.twoCol{display:grid;grid-template-columns:1.1fr .9fr;gap:10px}.topList,.mobileRows{margin-top:8px;display:grid;gap:6px}.topItem,.dayCard{border:1px solid var(--line);border-radius:14px;padding:8px 10px;background:#fff;display:flex;justify-content:space-between;align-items:center;gap:10px}.rank{min-width:26px;height:26px;border-radius:999px;display:inline-grid;place-items:center;background:#111827;color:#fff;font-size:12px;font-weight:950}.topMain{display:flex;gap:8px;align-items:center;min-width:0}.topName{font-weight:950;line-height:1.2;overflow:hidden;text-overflow:ellipsis}.topSub,.daySub{margin-top:2px;color:var(--muted);font-size:12px;font-weight:850;line-height:1.15}.topMeta,.daySales{color:var(--muted);font-size:13px;font-weight:900;text-align:right;white-space:nowrap}.rangeRow{margin-top:8px;display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:end}.field{display:grid;gap:5px}.label{font-size:12px;color:var(--muted);font-weight:900}.input{width:100%;padding:10px 11px;border-radius:12px;border:1px solid var(--line);background:#fff;font-weight:850}.totals{margin-top:10px;border-top:1px solid var(--line);padding-top:10px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.totalBox{border:1px solid var(--line);border-radius:14px;padding:9px;background:#fff}.totalLabel{color:var(--muted);font-size:12px;font-weight:900}.totalValue{margin-top:3px;font-size:14px;font-weight:950}.sectionHead{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center}.tableWrap{margin-top:8px;overflow:auto;border-radius:14px;border:1px solid var(--line);background:#fff}table{width:100%;border-collapse:collapse;min-width:720px}th,td{padding:11px;border-bottom:1px solid var(--line);text-align:left;font-size:14px}th{color:var(--muted);font-size:13px;font-weight:950;background:#fafafa}.tfoot{background:#f9fafb;font-weight:950}.muted{color:var(--muted)}.err{color:#b91c1c;font-weight:900;font-size:13px}.mobileRows{display:none}@media(max-width:900px){.summaryGrid,.twoCol{grid-template-columns:1fr}}@media(max-width:640px){.wrap{padding:10px;gap:8px}.h1{font-size:23px}.card{padding:10px}.summaryGrid{gap:8px}.salesLine{font-size:24px}.btn{padding:9px 11px}.notice{align-items:flex-start}.rangeRow{grid-template-columns:1fr 1fr}.rangeRow .btnPrimary{grid-column:1/-1}.totals{grid-template-columns:1fr 1fr;gap:6px}.tableWrap{display:none}.mobileRows{display:grid}}
      `}</style>

      <header className="topbar">
        <div className="titleRow">
          <div><h1 className="h1">매출 통계</h1><div className="metaRow" style={{ marginTop: 7 }}><span className="pill">{displayStoreName}</span><span className="pill">취소 주문 제외</span><span className="pill">{isPaidSubscriber ? "유료 구독 중" : "일부 제한"}</span><span className="pill">{modeLabel}</span></div></div>
          <div className="btnRow"><a className="btn" href="/admin">관리자 홈</a><button className="btn" type="button" onClick={fetchFromDb} disabled={loading}>{loading ? "새로고침 중" : "새로고침"}</button></div>
        </div>
        {errMsg ? <div className="err">오류: {errMsg}</div> : null}
      </header>

      {!isPaidSubscriber ? <section className="card notice"><p className="noticeText">일부 통계는 구독 후 확인할 수 있습니다.</p><button className="btn btnPrimary" type="button" onClick={goBilling}>구독 관리</button></section> : null}

      <section className="summaryGrid">
        {renderMetricCard("오늘", todayKey, daily)}
        {renderMetricCard("이번 주", `${weekStart} ~ ${weekEnd}`, weekly, !isPaidSubscriber)}
        <div className="card metricCard"><div className="cardHead"><div><h2 className="cardTitle">이번 달</h2><div className="cardPeriod">{month}</div></div>{isPaidSubscriber ? <span className="miniBadge">포장 {takeoutRate(monthly)}%</span> : <span className="lockBadge">상세 유료</span>}</div><div className="salesLine">{formatWonCompact(monthly.sales)}</div>{isPaidSubscriber ? <><div className="metricLine">주문 {monthly.orders}건 · 수량 {monthly.qty}개</div><div className="metricSub">객단가 {formatWon(avgOrder(monthly))} · 매장 {monthly.dineIn} / 포장 {monthly.takeout}</div></> : <div className="metricSub">상세 지표는 구독 후 확인</div>}</div>
      </section>

      <section className="twoCol">
        <div className="card"><div className="sectionHead"><h2 className="cardTitle">인기 메뉴 TOP 5</h2>{!isPaidSubscriber ? <span className="lockBadge">일부 공개</span> : null}</div>{top5.length === 0 ? <p className="muted" style={{ marginTop: 10 }}>당월 판매 데이터가 없습니다.</p> : <div className="topList">{(isPaidSubscriber ? top5 : top5.slice(0, 1)).map((m, idx) => <div key={m.id} className="topItem"><div className="topMain"><span className="rank">{idx + 1}</span><div><div className="topName">{m.name}</div><div className="topSub">판매 {m.qty}개</div></div></div><div className="topMeta">{isPaidSubscriber ? formatWon(m.sales) : "구독 후"}</div></div>)}{!isPaidSubscriber ? <div className="lockBox"><strong>나머지는 구독 후 확인</strong><button className="linkBtn" type="button" onClick={goBilling}>구독 관리</button></div> : null}</div>}</div>
        <div className="card"><div className="sectionHead"><h2 className="cardTitle">기간 조회</h2>{!isPaidSubscriber ? <span className="lockBadge">유료 전용</span> : null}</div><div className="quickRow" style={{ marginTop: 8 }}><button className="quickBtn" type="button" onClick={() => setPreset("today")} disabled={!isPaidSubscriber}>오늘</button><button className="quickBtn" type="button" onClick={() => setPreset("7days")} disabled={!isPaidSubscriber}>7일</button><button className="quickBtn" type="button" onClick={() => setPreset("month")} disabled={!isPaidSubscriber}>이번 달</button><button className="quickBtn" type="button" onClick={() => setPreset("lastMonth")} disabled={!isPaidSubscriber}>지난 달</button></div><div className="rangeRow"><label className="field"><span className="label">시작일</span><input className="input" type="date" value={rangeStart} disabled={!isPaidSubscriber} onChange={(e) => { setRangeStart(e.target.value); setRangeExpanded(false); }} /></label><label className="field"><span className="label">종료일</span><input className="input" type="date" value={rangeEnd} disabled={!isPaidSubscriber} onChange={(e) => { setRangeEnd(e.target.value); setRangeExpanded(false); }} /></label><button className="btn btnPrimary" type="button" onClick={downloadRangeCsv} disabled={loading}>상세 내역 받기</button></div>{isPaidSubscriber ? <div className="totals"><div className="totalBox"><div className="totalLabel">매출</div><div className="totalValue">{formatWon(rangeTotals.sales)}</div></div><div className="totalBox"><div className="totalLabel">주문</div><div className="totalValue">{rangeTotals.orders}건</div></div><div className="totalBox"><div className="totalLabel">수량</div><div className="totalValue">{rangeTotals.qty}개</div></div><div className="totalBox"><div className="totalLabel">포장</div><div className="totalValue">{takeoutRate(rangeTotals)}%</div></div></div> : <div className="lockBox"><strong>기간별 상세는 구독 후 확인</strong><button className="linkBtn" type="button" onClick={goBilling}>구독 관리</button></div>}</div>
      </section>

      <section className="card">
        <div className="sectionHead"><h2 className="cardTitle">일자별 요약</h2>{isPaidSubscriber && effectiveStart && effectiveEnd && effectiveStart <= effectiveEnd && hasMoreRange ? <button className="btn" type="button" onClick={() => setRangeExpanded((v) => !v)}>{rangeExpanded ? "접기" : "전체 보기"}</button> : null}</div>
        {!effectiveStart || !effectiveEnd ? <p className="muted" style={{ marginTop: 10 }}>기간을 선택해 주세요.</p> : effectiveStart > effectiveEnd ? <p className="muted" style={{ marginTop: 10 }}>시작일이 종료일보다 늦습니다.</p> : rangeSummaryRows.length === 0 ? <p className="muted" style={{ marginTop: 10 }}>선택한 기간에 주문이 없습니다.</p> : <><div className="tableWrap"><table><thead><tr><th>일자</th><th>매출</th><th>주문</th><th>수량</th><th>매장</th><th>포장</th></tr></thead><tbody>{visibleRangeRows.map((r) => <tr key={r.date}><td style={{ fontWeight: 900 }}>{r.date}</td><td>{formatWon(r.sales)}</td><td>{r.orders}건</td><td>{r.qty}개</td><td>{r.dineIn}</td><td>{r.takeout}</td></tr>)}</tbody><tfoot><tr className="tfoot"><td>합계</td><td>{formatWon(rangeTotals.sales)}</td><td>{rangeTotals.orders}건</td><td>{rangeTotals.qty}개</td><td>{rangeTotals.dineIn}</td><td>{rangeTotals.takeout}</td></tr></tfoot></table></div><div className="mobileRows">{visibleRangeRows.map((r) => <div key={r.date} className="dayCard"><div><strong>{r.date}</strong><div className="daySub">주문 {r.orders} · 수량 {r.qty} · 매장 {r.dineIn} / 포장 {r.takeout}</div></div><div className="daySales">{formatWon(r.sales)}</div></div>)}</div>{!isPaidSubscriber && hasMoreRange ? <div className="lockBox"><strong>최근 3일만 표시</strong><button className="linkBtn" type="button" onClick={goBilling}>구독 관리</button></div> : null}</>}
      </section>
    </main>
  );
}
...
export default function AdminStatsPage() {
  return <Suspense fallback={<div className="card"><p className="muted">로딩 중...</p></div>}><AdminStatsPageInner /></Suspense>;
}
