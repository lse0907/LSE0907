...
export default function SignupPage() {
  ...
  const onCompleteAddress = (data: Address) => {
    // data.address = 기본 선택 주소(도로명/지번 등)
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // 2) 일부 환경에서 signUp 직후 user/session이 비어 있을 수 있어
    //    (특히 설정에 따라) -> 안전하게 한번 로그인해서 세션 확보
    if (!signUpData?.user) {
    ...
    if (profileErr) {
      // 여기서 막히면 대부분 profiles RLS/정책/테이블 생성 문제
  ...
  return (
    <main style={{ maxWidth: 460, margin: "0 auto", padding: 24 }}>
      ...
      {msg ? (
        <p style={{ color: "#b91c1c", fontWeight: 900, marginTop: 12, whiteSpace: "pre-wrap" }}>
          {msg}
        </p>
      ) : null}
