...
-- No personal-data snapshots: the member reads their current profile through an ownership-checked API.
alter table public.privacy_rights_requests
  add column profile_access_granted boolean not null default false;

create index idx_privacy_rights_ops_queue
  on public.privacy_rights_requests(requested_at,id)
  where request_type in ('access','correction','deletion','restriction');
create index idx_privacy_events_rights_time
  on public.privacy_request_events(rights_request_id,occurred_at,id)
  where rights_request_id is not null;

create function public.ops_process_privacy_request(
...
begin
  if not exists (
    select 1 from auth.users where id=p_actor_id
      and raw_app_meta_data->>'role'='ops'
      and raw_app_meta_data->>'ops_role' in ('master','support')
  ) then raise exception 'OPS_PRIVACY_FORBIDDEN'; end if;
  ...
  ) then raise exception 'INVALID_PRIVACY_ACTION'; end if;
  ...
  if p_action in ('review','identity_required','reject') then
    ...
    if p_action='provide_profile_access' then
      ...
      if p_action='correct_customer_name' then
        ...
        v_result := '고객 서비스의 선택 전화번호를 정정했습니다. 로그인 인증번호나 주문별 연락처는 변경하지 않았습니다.';
      else
  ...
  -- Mutation, result and audit succeed or roll back together. Never store before/after personal values in audit metadata.
